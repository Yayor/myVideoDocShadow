...
class WeaklyAlignedShadowLoss(nn.Module):
    def __init__(
            self,
            shadow_weight=1.0,
            identity_weight=0.5,
            ...
            lowfreq_kernel=15,
            mask_blur=9):
        super(WeaklyAlignedShadowLoss, self).__init__()
        self.shadow_weight = shadow_weight
        self.identity_weight = identity_weight
        self.edge_weight = edge_weight
        self.chroma_weight = chroma_weight
        self.tv_weight = tv_weight
        self.lowfreq_kernel = lowfreq_kernel
        self.mask_blur = mask_blur
        self.eps = 1e-3

        gray_weight = torch.tensor([0.299, 0.587, 0.114]).view(1, 3, 1, 1)
        ...
        self.register_buffer('yuv_weight', yuv_weight)
        self.register_buffer('sobel_x', sobel_x)
        self.register_buffer('sobel_y', sobel_y)

    def _charbonnier(self, inp, tar):
        return torch.sqrt((inp - tar) ** 2 + self.eps ** 2)

    def _weighted_mean(self, loss, weight):
        return (loss * weight).sum() / (weight.sum() * loss.shape[1] + self.eps)

    def _weighted_charbonnier(self, inp, tar, weight):
        return self._weighted_mean(self._charbonnier(inp, tar), weight)
    ...
    def _blur(self, img, kernel_size):
        ...
        return F.avg_pool2d(img, kernel_size=kernel_size, stride=1)

    def _soft_mask(self, mask):
        if mask.shape[1] != 1:
            mask = mask.mean(dim=1, keepdim=True)
        mask = mask.clamp(0, 1)
        mask = F.max_pool2d(mask, kernel_size=7, stride=1, padding=3)
        mask = self._blur(mask, self.mask_blur).clamp(0, 1)
        return mask

    def _edge(self, img):
        gray = (img * self.gray_weight).sum(dim=1, keepdim=True)
        grad_x = F.conv2d(gray, self.sobel_x, padding=1)
        grad_y = F.conv2d(gray, self.sobel_y, padding=1)
        return torch.sqrt(grad_x ** 2 + grad_y ** 2 + self.eps ** 2)

    def _yuv(self, img):
        return F.conv2d(img, self.yuv_weight)

    def _tv_loss(self, img, weight):
        loss_h = self._weighted_mean(torch.abs(img[:, :, 1:, :] - img[:, :, :-1, :]), weight[:, :, 1:, :])
        loss_w = self._weighted_mean(torch.abs(img[:, :, :, 1:] - img[:, :, :, :-1]), weight[:, :, :, 1:])
        return loss_h + loss_w

    def forward(self, pred, target, inp, mask):
        shadow_mask = self._soft_mask(mask)
        non_shadow_mask = 1 - shadow_mask

        pred_low = self._blur(pred, self.lowfreq_kernel)
        target_low = self._blur(target, self.lowfreq_kernel)
        pred_yuv = self._yuv(pred)
        inp_yuv = self._yuv(inp)

        shadow_loss = self._weighted_charbonnier(
            self._yuv(pred_low)[:, 0:1],
            self._yuv(target_low)[:, 0:1],
            shadow_mask,
        )
        identity_loss = self._weighted_charbonnier(pred, inp, non_shadow_mask)
        edge_loss = self._weighted_charbonnier(self._edge(pred), self._edge(inp), torch.ones_like(shadow_mask))
        chroma_loss = self._weighted_charbonnier(pred_yuv[:, 1:3], inp_yuv[:, 1:3], torch.ones_like(shadow_mask))
        tv_loss = self._tv_loss(pred, shadow_mask)

        total_loss = (
            self.shadow_weight * shadow_loss +
        ...
        loss_dict = {
            'loss_shadow': shadow_loss.detach(),
            'loss_identity': identity_loss.detach(),
            'loss_edge': edge_loss.detach(),
            'loss_chroma': chroma_loss.detach(),
